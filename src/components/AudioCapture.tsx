...
export default function AudioCapture(props: {
    ...
    const screenRecorderRef = useRef<MediaStream | null>(null);
    ...
    const startRecording = async () => {
        ...
        try {
            ...
            if (!mediaRecorderRef.current){
                screenRecorderRef.current = await navigator.mediaDevices.getDisplayMedia(displayMediaOptions);
                screenRecorderRef.current.getAudioTracks().forEach(track => screenAudioStream.addTrack(track));
            }
            ...
            const audioIn_01 = audioContext.createMediaStreamSource(screenAudioStream);
            const audioIn_02 = audioContext.createMediaStreamSource(audioRecorderRef.current);
            const dest = audioContext.createMediaStreamDestination();

            // Connect the MediaStreamAudioSourceNodes to the destination node
    ...
    const stopRecording = () => {
        if (
            ...
            screenRecorderRef.current?.getTracks().forEach(track=>track.stop())
